...
module.exports = (req, res, next) => {
    ...
    if (id) {
        if (!Object.keys(req.body).length) throw new clientError("Params to update not found", 400);
        else next();
    }
    ...
    if (model === models.Character) {
        ...
        if (!id || !title || !planets || !characters) {
            throw new clientError("id, title, planets and characters are required", 400);
        } else next();
    } else {
